...
describe('Parsers', function () {
  ...
  describe('countDeclarations', function () {
    it('should return the number of declarations in output', function () {
      assert.equal(parsers.countDeclarations(ast), 7);
    });

    it('should return 0 if there are no declarations', function () {
      assert.equal(parsers.countDeclarations({
        stylesheet: {
          rules: [
            {
              declarations: []
  ...
  describe('findDeclarations', function () {
    it('should return the declaration object array if the property is found', function () {
      var declaration = [
        {
          "type": "declaration",
          "property": "max-height",
      ...
      assert.deepEqual(parsers.findDeclarations(ast, 'max-height'), declaration);
    });
    it('should return the declaration object array if the property is found inside a media query', function() {
      var declaration = [
        {
          "type": "declaration",
          "property": "max-height",
      ...
      assert.deepEqual(parsers.findDeclarations(astMediaQuery, 'max-height'), declaration);
    })
  });
  
  describe('findDeclarationInSelector', function() {
    it('should return the declaration Object found in the specified selector when @at-root is being used', function () {
       var declaration = {
        'type': 'declaration',
        'property': 'color',
        'value': 'red',
      ...
      assert.deepEqual(parsers.findDeclarationInSelector(ast, '@at-root .testInSelector', 'color'), declaration);
    })

    it('should return the declaration Object found in the specified selector when & selector is used', function () {
       var declaration = {
        'type': 'declaration',
        'property': 'color',
        'value': 'red',
      ...
      assert.deepEqual(parsers.findDeclarationInSelector(ast, '&.testInSelector', 'color'), declaration);
    })
  });
